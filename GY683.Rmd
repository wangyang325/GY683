...
```{r echo=FALSE}
p1<-ggplot(MyData, aes(y=Purprice))+ geom_boxplot()
MyData <- MyData[MyData$Purprice < 600000,]
p2<-ggplot(MyData, aes(y=Purprice))+ geom_boxplot()
...
Then we want to find the predictor that has the most impact on price. So, we used AIC to compare the different predictors. Then fit model with all predictors and choose significant predictors for linear model. Finally, fit model with significan predictors and check VIF of predictors to avoid colinearity.
